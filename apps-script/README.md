...
| E | 出席類型 | `attend_type` | full→觀禮＋婚宴／ceremony-only→僅觀禮／banquet-only→僅婚宴／gift-only→禮到人不到／absent→無法出席 |
...
「統計」工作表的公式都是照這個欄位順序、比對中文值寫的（見 `Code.gs` 的 `setupStatsSheet`）。出席類型五選一（觀禮＋婚宴／僅觀禮／僅婚宴／禮到人不到／無法出席），只有「觀禮＋婚宴」「僅婚宴」這兩種有參加婚宴，才會有人數／飲食／兒童椅／餐具資料；「僅觀禮」只留姓名/關係/電話/備註，另外獨立一份名單。版面配置：

| 區塊 | 欄位範圍 | 內容 |
| :-- | :-- | :-- |
| 統計數字 | A:C | 婚宴出席組數／人數、僅觀禮組數、各飲食人數、兒童椅／餐具張數 |
| 喜餅郵寄清單 | E:G | 姓名 / 電話 / 地址（喜餅領取＝郵寄） |
| 婚宴出席名單 | I:P | 姓名、關係、人數、飲食、兒童椅、餐具、電話、備註（出席類型＝觀禮＋婚宴或僅婚宴，依填寫時間排序） |
| 禮到人不到／無法出席名單 | R:V | 姓名、關係、出席類型、喜餅領取、電話（出席類型＝禮到人不到或無法出席且姓名不為空） |
| 僅觀禮名單 | X:AA | 姓名、關係、電話、備註（出席類型＝僅觀禮，依填寫時間排序） |

五塊分別佔用不同欄位區間，各自往下長不會互相覆蓋。
